...
const ThreadCommentPointRepository = dataSource
  .getRepository(ThreadCommentPoint)
  .extend({
    async togglePoint(
      userId: string,
      commentId: string,
      ...
      if (userId === comment.author.id) {
        throw new Error(
          'Thread comment author cannot like/dislike own threads',
        );
      }

      const existingPoint = await this.getPointByUserAndThreadId(
        userId,
        commentId,
      );
      ...
      return await this.manager.transaction(
        async (
          ...
          if (type === existingPoint.type) {
            ...
            throw new Error(
              `You have already ${prefix}voted this comment, undo your previous vote first`,
            );
          }
        },
      );
    ...
    async getPointById(id: string): Promise<ThreadCommentPoint | null> {
      if (!id) {

...
import { ThreadPoint } from '../persistence/entities';
...
const ThreadPointRepository = dataSource.getRepository(ThreadPoint).extend({
    ...
    return await this.manager.transaction(
      async (
        ...
        if (type === existingPoint.type) {
          ...
          throw new Error(
            `You have already ${prefix}voted this thread, undo your previous vote first`,
          );
        }
      },
    );
  ...
  async getPointById(id: string): Promise<ThreadPoint | null> {
    if (!id) {
